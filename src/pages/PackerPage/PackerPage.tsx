...
export const PackerPage: React.FC = () => {
  ...
  const [selectedPacker, setSelectedPacker] = useState<IPacker | null>(
    packerSavedInLocal[0]
  );
  ...
  const { data: intDocs, isSuccess: isSuccessIntDocs } = useIntDocs(
    {
      page: query.search ? 1 : currentPage,
      limit: perPage,
      filter: query.filter,
      search: query.search,
    },
  ...
  const { data: packersData, refetch } = usePackers();
      ...
      <div className={s.table_container}>
        <div className={s.form_container}>
          ...
          <div className={s.filter_container}>
            <SearchPackerForm
              onSearch={(search, filter) => setQuery({ search, filter })}
            />
          </div>
        </div>
